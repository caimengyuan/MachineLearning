...
def chooseBestFeatureToSplit(dataSet):
    ...
    for i in range(numFeatures):        # 遍历所有特征
        ...
        for value in uniqueVals:        # 计算信息增益
            subDataSet  = splitDataSet(dataSet, i, value)       # sudDataSet划分后的子集
...
def majorityCnt(classList):
    ...
    for vote in classList:      # 统计classList中每个元素出现的次数
        if vote not in classCount.keys():
            classCount[vote] = 0
    # 将字典分解为元组列表，根据第二个属性值进行排序
    sortedClassCount = sorted(classCount.items(), key=operator.itemgetter(1), reverse=True)     # 根据字典的值降序排序
    return sortedClassCount[0][0]       # 返回classList中出现次数最多的元素

# 创建决策树
def createTree(dataSet, labels, featLabels):        # dataSet - 训练数据集，labels - 分类属性标签，featLabels - 存储选择的最优特征标签
    classList = [example[-1] for example in dataSet]        # 取分类标签（是否放贷yes or no)
...
def plotMidText(cntrPt, parentPt, txtString):
    xMid = (parentPt[0] - cntrPt[0])/2.0 + cntrPt[0]        #   计算标柱位置
...
def plotTree(myTree, parentPt, nodeTxt):
    ...
    for key in secondDict.keys():
        if type(secondDict[key]).__name__=='dict':      # 测试该结点是否为字典，如果不是字典，代表此结点为叶子
            plotTree(secondDict[key], cntrPt, str(key))         # 不是叶结点递归调用继续绘制
...
def createPlot(inTree):
    ...
    #去掉x,y轴 111表示fig中的图有一行一列，最后的1表示一个图，frameon表示是否绘制坐标轴
